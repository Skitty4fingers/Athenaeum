...
- **Installable, but the mobile experience itself isn't the focus.** A web manifest and service worker (`public/manifest.json`, `public/sw.js`) make it a real installable PWA — "Add to Home Screen" on iOS/Android, a standalone window with no browser chrome, and (as of the offline-listening work above) the app shell itself caches on first visit so it can cold-load with no connectivity at all. What's still true: no expanded mobile Now Playing, no dedicated install-prompt UI (the browser's own native affordance covers it), and background-audio behavior on mobile Safari/Chrome hasn't been specifically tuned — MediaSession/lock-screen controls exist (see [Playback](#playback)), but nothing beyond that. Phones are meant to use the official app or a compatible client, which this server still serves.
